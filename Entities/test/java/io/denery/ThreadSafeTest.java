...
class ThreadSafeTest {
    // Don't make fields final, otherwise sequential consistency may change!
    private Location location = new Location(null, 0, 60, 0);


    @Test
    void locationGetThreadSafeTest() {
    ...
    @Test
    void worldBlockAtThreadSafeTest() {
        CompletableFuture.runAsync(() -> {
            try {
                System.out.println(dummyWorld.getBlockAt(0, 0, 0).isSolid());
            } catch (Exception e) {
                e.printStackTrace();
            }
        });
